...
def processing(input_path, output_file_path, frame, labels, crop_xy, crop_z,method,raw):
    """
    ...
    # im = im[frame,:,...].compute()
    im = np.expand_dims(im, axis=2)  # Ensure im has a channel dimension
    channel = 0
...
def processing_chunk(chunk):
    ...
    for args in chunk:
        ...
        input_path, output_file_path, frame, labels, crop_xy, crop_z,method,raw = args
        # Process each item in the chunk
        result = processing(input_path, output_file_path, frame, labels, crop_xy, crop_z,method,raw)
...
def main():
    ...
    tasks = [
    (input_path,f"{output_path}_{frame}_.parquet", frame, labels, args.crop_size_xy, args.crop_size_z,method,raw)
