...
def find_tracks_to_refine_no_beads(track1,track2,cutoff=1.5,proportion_good_track=0.8,cutoff_jump_single_color=0.5):
...
def correct_track_no_beads(track1,track2,df,label,snr1,snr2,pixel_size,cutoff=1.5,cutoff_jump_single_color=2):
    ...
    for i in range(0,len(track1)-1):
        ...
        elif(track1[i][2]==track1[i][-1] or track2[i][2]==track2[i][-1]):
            track1[i]=[0,0,0,0,0,0]
            track2[i]=[0,0,0,0,0,0]
            snr1[i] = [0,0,0]
            snr2[i] = [0,0,0]

        elif(track1[i][0]!=0 and track1[i+1][0]==0):
            if(i==0):
                track1[i]=[0,0,0,0,0,0]
                track2[i]=[0,0,0,0,0,0]
                snr1[i] = [0,0,0]
                ...
                if(track1[i-1][0]==0):
                    ...
                    snr1[i] = [0,0,0]
                    snr2[i] = [0,0,0]
                elif(d1>cutoff_jump_single_color or d2>cutoff_jump_single_color):
                    track1[i]=[0,0,0,0,0,0]
                    track2[i]=[0,0,0,0,0,0]
                    snr1[i] = [0,0,0]
                    snr2[i] = [0,0,0]
            ...
            if(d1>cutoff_jump_single_color or d2>cutoff_jump_single_color):
                if(i==0):
                    ...
                    if(track1[i-1][0]==0):
                        ...
                        snr1[i] = [0,0,0]
                        snr2[i] = [0,0,0]
                    elif(d1<cutoff_jump_single_color and d2<cutoff_jump_single_color):
...
def process_df_no_beads(path_run_folder,cutoff=1.5,proportion_good_track=1.0,cxy=9,cz=7,pixel_sizeinit=[0.13,0.13,0.3],raw=False,method='gauss',cutoff_single_channel=1,frame_to_cut=10):
    ...
    try:
        ...
        for track in df.new_label.unique():
            ...
            temp_traj_2[d[(d.new_label == track)&(d.channel == 1)]['frame'].values.astype(int)-frame_to_cut] = d[(d.new_label == track)&(d.channel == 1)][['x', 'y','z','x_fitted_refined','y_fitted_refined','z_fitted_refined']].values*pixel_size
            snr_c2_temp[d[(d.new_label == track)&(d.channel == 1)]['frame'].values.astype(int)-frame_to_cut] = d[(d.new_label == track)&(d.channel == 1)][['snr_tophat',"sigma_xy","sigma_z"]]
