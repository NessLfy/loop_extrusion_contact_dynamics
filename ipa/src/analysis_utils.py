...
def find_tracks_to_refine(track1,track2,model):
    t_1 = np.array(track1)
    t_2 = np.array(track2)
    ...
    if len(dist_full[(dist_full < cutoff) & (dist_full > 0)])/len(dist_full) > 0.8:
        return True
    else:
        return False
...
def process_df(path_run_folder):
    ...
    for track in d.new_label.unique():
        temp_traj_1 = np.zeros((480, 3))
        temp_traj_2 = np.zeros((480, 3))
        snr_c1_temp = np.zeros((480,1))
        snr_c2_temp = np.zeros((480,1))
        ...
        if find_tracks_to_refine(temp_traj_1,temp_traj_2,model):
            track1, track2,dist = correct_track(temp_traj_1,temp_traj_2,model,df,track)
